...
class Enemie extends Box {
    constructor({ width, height, depht, colorP = '#FA5858', x = 0, y = 0, z = 0 }) {
...
class Enemie2 extends Box {
    constructor({ width, height, depht, colorP = '#58FA58', x = 0, y = 0, z = 0 }) {
        super({ width: width, height: height, depht: depht, colorP: colorP, x: x, y: y, z: z });
        ...
        this.corpo = new Sphere({ radius: 0.5, colorP: colorP });
        this.corpo.position.x = this.position.x;
        this.corpo.position.y = this.position.y;
        this.corpo.position.z = this.position.z;
        //cima
        this.spikeUp = new Cone({ radius: 0.5, height: height, colorP: colorP });
        this.spikeUp.position.x = this.position.x;
        this.spikeUp.position.y = this.position.y + this.height / 2;
...
let sol = new Sun({ color: '#ffffff', intensity: 1, x: 0, y: 5, z: 10 });
cena.add(sol);
let sol2 = new Sun({ color: '#ffffff', intensity: 1, x: 0, y: -5, z: -10 });
cena.add(sol2);
sol2.lookAt(new THREE.Vector3(0, 0, 0));
sol.lookAt(new THREE.Vector3(0, 0, 0));
...
let light4E = new THREE.PointLight({ color: "#ffffff", intensity: 0.1 })


light4E.position.x = -6;
light4E.position.y = -2;
light4E.position.z = 0;
...
function Update() {
    ...
    if (lPressed && !lWasPressed) {
        ...
        light4E.visible = !light4E.visible;
        ...
        for (var i = 0; i < nrObjects; i++) {
            ...
            if (rad > 20 && rad <= 95) {
                if (counter < maxEnemies) {
                    var prob = THREE.MathUtils.randInt(0, 100);
                    if (prob >= 50) {
                        var enemie = new Enemie({ width: 1, height: 1, depht: 1, colorP: "#FA5858", x: x_pos, y: -1.5, z: -15 });
                        cena.add(enemie);
                        enemie.addToScene(cena);
                        ListOfCollindingEnimieMesh.push(enemie);
                    }else{
                        var enemie = new Enemie2({ width: 1, height: 1, depht: 1, colorP: "#58FA58", x: x_pos, y: -1.5, z: -15 });
                        cena.add(enemie);
                        enemie.addToScene(cena);
                        ListOfCollindingEnimieMesh.push(enemie);
                    }
